...
def analizar_ortografia(texto, idiomas=['es', 'fr', 'en', 'ca','ca-general','ca-valencia']):
    ...
    if idioma_detectado and idioma_detectado in idiomas:
        try:
            ...
            palabras_excluidas = {'MUTUAL', 'MC-MUTUAL', 'cookies',
                'personalizables','Home','MUTUAL','psicosocial','personalizables','sostenibilidad','Home','ICDQ',
                'Cyclops','Midat','MCIT','online'
            }
            ...
            comunidades_autonomas_espanolas = {
                ...
                'pais vasco', 'la rioja', 'comunidad valenciana','MUTUAL', 'MC-MUTUAL', 'cookies',
                'personalizables','Home','MUTUAL','psicosocial','personalizables','sostenibilidad','Home','ICDQ',
                'Cyclops','Midat','MCIT','online'
            }
...
def analizar_heading_tags(response_text):
    ...
    h2_mas_70_caracteres = sum(len(tag.text) > 70 for tag in soup.find_all('h2'))
    ...
    h2_no_secuencial = heading_tags_count.get('h1', 0) > 0 and heading_tags_count.get('h2', 0) > 0 and soup.find_all('h1')[0].find_next('h2') is None
...
def extraer_meta_tags(response_text, response):
    ...
    meta_description_mas_155_caracteres = any(len(tag.get('content', '')) > 155 for tag in meta_tags if tag.get('name') == 'description')
    meta_description_duplicado = any(meta_tags_info.count(tag) > 1 for tag in meta_tags_info if tag.get('name') == 'description')

    canonicals_falta = not soup.find('link', {'rel': 'canonical'})
    directivas_noindex = 'noindex' in [tag.get('content') for tag in meta_tags if tag.get('name') == 'robots']

    falta_encabezado_x_content_type_options = 'X-Content-Type-Options' not in [header[0] for header in response.headers]
    falta_encabezado_secure_referrer_policy = 'Referrer-Policy' not in [header[0] for header in response.headers]
    falta_encabezado_content_security_policy = 'Content-Security-Policy' not in [header[0] for header in response.headers]
    falta_encabezado_x_frame_options = 'X-Frame-Options' not in [header[0] for header in response.headers]

    titulos_pagina_menos_30_caracteres = any(len(tag.text) < 30 for tag in soup.find_all(['h1', 'h2', 'h3', 'h4', 'h5', 'h6']))
    meta_description_menos_70_caracteres = any(len(tag.get('content', '')) < 70 for tag in meta_tags if tag.get('name') == 'description')
    titulos_pagina_mas_60_caracteres = any(len(tag.text) > 60 for tag in soup.find_all(['h1', 'h2', 'h3', 'h4', 'h5', 'h6']))
    titulos_pagina_igual_h1 = any(tag.text == soup.find('h1').text for tag in soup.find_all(['h2', 'h3', 'h4', 'h5', 'h6']))
    titulos_pagina_duplicado = any(meta_tags_info.count(tag) > 1 for tag in meta_tags_info if tag.get('name') == 'title')

    meta_description_falta = not any(tag.get('name') == 'description' for tag in meta_tags)
...
def contar_enlaces(response_text):
    ...
    # Enlaces internos
    enlaces_internos = [enlace['href'] for enlace in enlaces if not enlace['href'].startswith(('http://', 'https://'))]
    ...
    # Enlaces salientes
    enlaces_salientes = [enlace['href'] for enlace in enlaces if enlace.get('href') and enlace['href'].startswith(('http://', 'https://'))]
    ...
    enlaces_salientes_externos = [enlace for enlace in enlaces_salientes if not any(enlace.startswith(dominio) for dominio in DOMINIOS_ESPECIFICOS)]
    ...
    enlaces_salientes_js_externos_unicos = [enlace for enlace in enlaces_salientes_js_unicos if not any(enlace.startswith(dominio) for dominio in DOMINIOS_ESPECIFICOS)]
    
    return (
        len(enlaces),
        len(enlaces_inseguros),
        len(enlaces_internos),
        len(enlaces_internos_unicos),
        len(enlaces_js_unicos),
        len(enlaces_salientes),
        len(enlaces_salientes_unicos),
        len(enlaces_salientes_js_unicos),
        len(enlaces_salientes_externos),
...
def analizar_meta_tags(response_text):
    ...
    meta_tags_info['c_robots'] = robots_tag.get('content') if robots_tag else ''
    meta_tags_info['c_description'] = description_tag.get('content') if description_tag else ''
    meta_tags_info['c_keywords'] = keywords_tag.get('content') if keywords_tag else ''
    meta_tags_info['c_charset'] = 'utf-8' #charset_tag.get('charset') if charset_tag else ''
...
def ejecutar_pa11y(url_actual):
    ...
    #    print(error_message)
        return [] #[{"type": "pa11y error", "code": "execution-error", "message": error_message, "context": "", "selector": ""}]
...
def escanear_dominio(url_dominio, exclusiones=[], extensiones_excluidas=[]):
    ...
    while urls_por_escanear:
        ...
        try:
            ...
            #    if es_html_valido(response.text):
                    ...
                    if response.headers['content-type'].startswith('text/html'):
                        if es_html_valido(response.text):
                            ...
                            resultados_pagina['enlaces_internos'] = 0  # Inicializar en 0
                            resultados_pagina['enlaces_internos_unicos'] = 0  # Inicializar en 0
                            resultados_pagina['enlaces_js_unicos'] = 0  # Inicializar en 0
                            resultados_pagina['enlaces_salientes'] = 0  # Inicializar en 0
                            resultados_pagina['enlaces_salientes_unicos'] = 0  # Inicializar en 0
                            resultados_pagina['enlaces_salientes_js_unicos'] = 0  # Inicializar en 0
                            resultados_pagina['enlaces_salientes_externos'] = 0  # Inicializar en 0
                            resultados_pagina['enlaces_salientes_externos_unicos'] = 0  # Inicializar en 0
                            resultados_pagina['enlaces_salientes_js_externos_unicos'] = 0  # Inicializar en 0 
                            ...
                            heading_tags_count, h1_duplicate, h2_mas_70_caracteres, h2_duplicado, h2_multiple, h2_falta, h2_no_secuencial = analizar_heading_tags(response.text)
                            ...
                            resultados_pagina['heading_tags'] = heading_tags_count or {}
                            resultados_pagina['h1_duplicate'] = h1_duplicate
                            resultados_pagina['h2_mas_70_caracteres'] = h2_mas_70_caracteres
                            resultados_pagina['h2_duplicado'] = h2_duplicado
                            resultados_pagina['h2_multiple'] = h2_multiple
                            resultados_pagina['h2_falta'] = h2_falta
                            resultados_pagina['h2_no_secuencial'] = h2_no_secuencial

                            resultados_pagina['meta_tags'] = meta_tags_info or {}
                            resultados_pagina['meta_description_mas_155_caracteres'] = meta_description_mas_155_caracteres
                            resultados_pagina['meta_description_duplicado'] = meta_description_duplicado
                            resultados_pagina['canonicals_falta'] = canonicals_falta
                            resultados_pagina['directivas_noindex'] = directivas_noindex
                            resultados_pagina['falta_encabezado_x_content_type_options'] = falta_encabezado_x_content_type_options
                            resultados_pagina['falta_encabezado_secure_referrer_policy'] = falta_encabezado_secure_referrer_policy
                            resultados_pagina['falta_encabezado_content_security_policy'] = falta_encabezado_content_security_policy
                            resultados_pagina['falta_encabezado_x_frame_options'] = falta_encabezado_x_frame_options
                            resultados_pagina['titulos_pagina_menos_30_caracteres'] = titulos_pagina_menos_30_caracteres
                            resultados_pagina['meta_description_menos_70_caracteres'] = meta_description_menos_70_caracteres
                            resultados_pagina['titulos_pagina_mas_60_caracteres'] = titulos_pagina_mas_60_caracteres
                            resultados_pagina['titulos_pagina_igual_h1'] = titulos_pagina_igual_h1
                            resultados_pagina['titulos_pagina_duplicado'] = titulos_pagina_duplicado
                            resultados_pagina['meta_description_falta'] = meta_description_falta
                            resultados_pagina['version_http'] = version_http
                            resultados_pagina['ultima_modificacion'] = ultima_modificacion
                            ...
                            info_imagenes, image_types, images_1MB, imagenes_rotas = extraer_informacion_imagenes(response.text, url_actual) #images_1MB
                            ...
                            enlaces_totales, enlaces_inseguros, enlaces_internos, enlaces_internos_unicos, enlaces_js_unicos, enlaces_salientes, enlaces_salientes_unicos, enlaces_salientes_js_unicos, enlaces_salientes_externos, enlaces_salientes_externos_unicos, enlaces_salientes_js_externos_unicos = contar_enlaces(response.text)
                            ...
                            resultados_pagina['enlaces_internos'] = enlaces_internos
                            resultados_pagina['enlaces_internos_unicos'] = enlaces_internos_unicos
                            resultados_pagina['enlaces_js_unicos'] = enlaces_js_unicos
                            resultados_pagina['enlaces_salientes'] = enlaces_salientes
                            resultados_pagina['enlaces_salientes_unicos'] = enlaces_salientes_unicos
                            resultados_pagina['enlaces_salientes_js_unicos'] = enlaces_salientes_js_unicos
                            resultados_pagina['enlaces_salientes_externos'] = enlaces_salientes_externos
                            resultados_pagina['enlaces_salientes_externos_unicos'] = enlaces_salientes_externos_unicos
                            resultados_pagina['enlaces_salientes_js_externos_unicos'] = enlaces_salientes_js_externos_unicos


                            resultados_pagina['images_1MB'] = images_1MB
                            resultados_pagina['imagenes_rotas'] = imagenes_rotas
            ...
            enlaces_href = [urljoin(url_actual, a['href']) for a in soup.find_all('a', href=True)]

            # Extraer enlaces con etiqueta 'src'
            enlaces_src = [urljoin(url_actual, img['src']) for img in soup.find_all('img', src=True)]
...
def generar_informe_resumen(resumen, nombre_archivo):
    ...
    with open(nombre_archivo, 'a', newline='', encoding='utf-8') as archivo_csv:
        ...
        for dominio, datos in resumen.items():
            ...
            total_404 = datos['total_404'] #sum(pagina.get('codigo_respuesta', 0) == 404 for pagina in datos.get('paginas', []))
            tiempo_medio = datos['tiempo_medio'] # (sum(pagina.get('tiempo_respuesta', 0) for pagina in datos.get('paginas', []))) / (total_paginas)
            pages_err_orto = 0
            pages_alt_vacias = 0
...
def guardar_en_csv_y_json(resultados, nombre_archivo_base, modo='w'):
    campos = ['fecha_escaneo', 'dominio', 'codigo_respuesta', 'tipo_documento', 'tiempo_respuesta', 'pagina', 'parent_url',
              ...
              'desc_short', 'h1_duplicate', 'images_1MB','imagenes_rotas','id_escaneo','alt_vacias','peso_total_pagina','is_pdf',
              'c_title','c_robots','c_description','c_keywords','c_charset',
              'frases','media_palabras_frase','flesh_score',
              'enlaces_internos','enlaces_internos_unicos','enlaces_js_unicos','enlaces_salientes','enlaces_salientes_unicos',
              'enlaces_salientes_js_unicos','enlaces_salientes_externos','enlaces_salientes_externos_unicos','enlaces_salientes_js_externos_unicos',
              'h2_mas_70_caracteres','h2_duplicado','h2_multiple','h2_falta','h2_no_secuencial',
              'meta_description_mas_155_caracteres','meta_description_duplicado','canonicals_falta','directivas_noindex',
              'falta_encabezado_x_content_type_options','falta_encabezado_secure_referrer_policy',
              'falta_encabezado_content_security_policy','falta_encabezado_x_frame_options',
              'titulos_pagina_menos_30_caracteres','meta_description_menos_70_caracteres','titulos_pagina_mas_60_caracteres',
              'titulos_pagina_igual_h1','titulos_pagina_duplicado','meta_description_falta',
              'version_http','ultima_modificacion','meta_og_card','meta_og_title','meta_og_image'
              ]
...
from config import DOMINIOS_ESPECIFICOS, USER, PWD, HOST, DB, PATRONES_EXCLUSION, EXTENSIONES_EXCLUIDAS
...
if __name__ == "__main__":
    ...
    engine = create_engine('mysql+mysqlconnector://'+USER+':'+PWD+'@'+HOST+'/'+DB, pool_size=10, max_overflow=20, pool_pre_ping=True, pool_recycle=3600, echo=False)  # Cambia 'echo=True' a 'False' para desactivar el modo verbose
        ...
        for url in urls_a_escanear:
            ...
            if parsed_url.netloc:
                ...
                resumen_escaneo[dominio] = {
                    ...
                    'media_flesh_score' : 0,
                    'total_meta_description_mas_155_caracteres' : 0,
                    'total_meta_description_duplicado' : 0,
                    'total_canonicals_falta' : 0,
                    'total_directivas_noindex' : 0,
                    'total_falta_encabezado_x_content_type_options' : 0,
                    'total_falta_encabezado_secure_referrer_policy' : 0,
                    'total_falta_encabezado_content_security_policy' : 0,
                    'total_falta_encabezado_x_frame_options' : 0,
                    'total_titulos_pagina_menos_30_caracteres' : 0,
                    'total_meta_description_menos_70_caracteres' : 0,
                    'total_titulos_pagina_mas_60_caracteres' : 0,
                    'total_titulos_pagina_igual_h1' : 0,
                    'total_titulos_pagina_duplicado' : 0,
                    'total_meta_description_falta' : 0,
                    'total_h2_duplicado' : 0,
                    'total_h2_mas_70_caracteres' : 0,
                    'total_h2_multiple' : 0,
                    'total_h2_falta' : 0,
                    'total_h2_no_secuencial' : 0
                }
        ...
        with Session() as session:
          with session.no_autoflush:
            for dominio, resumen in resumen_escaneo.items():
                ...
                suma_media_frases = session.query(func.sum(Resultado.frases)).filter(
                    Resultado.dominio == dominio,
                ...
                suma_media_palabras_frase = session.query(func.sum(Resultado.media_palabras_frase)).filter(
                    Resultado.dominio == dominio,
                    Resultado.codigo_respuesta == 200,
                    Resultado.id_escaneo == most_recent_id_escaneo,
                ...
                suma_flesh_scores = session.query(func.sum(Resultado.flesh_score)).filter(
                    Resultado.dominio == dominio,
                    Resultado.codigo_respuesta == 200,
                    Resultado.id_escaneo == most_recent_id_escaneo,
                ...
                total_meta_description_mas_155_caracteres = len(resultados_tmp_20)
                
                print(f"total_meta_description_mas_155_caracteres: {total_meta_description_mas_155_caracteres}")
                ...
                print(f"total_meta_description_duplicado: {total_meta_description_duplicado}")
                ...
                print(f"total_directivas_noindex: {total_directivas_noindex}")
                ...
                total_falta_encabezado_x_content_type_options = len(resultados_tmp_16)  
                
                print(f"total_falta_encabezado_x_content_type_options: {total_falta_encabezado_x_content_type_options}")
                ...
                total_falta_encabezado_secure_referrer_policy = len(resultados_tmp_15) 
                
                print(f"total_falta_encabezado_secure_referrer_policy : {total_falta_encabezado_secure_referrer_policy }")   
                ...
                total_falta_encabezado_content_security_policy = len(resultados_tmp_14)    
                
                print(f"total_falta_encabezado_content_security_policy : {total_falta_encabezado_content_security_policy}") 
                ...
                total_falta_encabezado_x_frame_options = len(resultados_tmp_13) 
                
                print(f"total_falta_encabezado_x_frame_options : {total_falta_encabezado_x_frame_options}") 
                ...
                total_titulos_pagina_menos_30_caracteres = len(resultados_tmp_12) 
                
                print(f"total_titulos_pagina_menos_30_caracteres : {total_titulos_pagina_menos_30_caracteres}")
                ...
                total_meta_description_menos_70_caracteres = len(resultados_tmp_11) 
                
                print(f"total_meta_description_menos_70_caracteres : {total_meta_description_menos_70_caracteres}")
                ...
                total_titulos_pagina_mas_60_caracteres = len(resultados_tmp_10) 
                
                print(f"total_titulos_pagina_mas_60_caracteres  : {total_titulos_pagina_mas_60_caracteres}")
                ...
                print(f"total_titulos_pagina_igual_h1  : {total_titulos_pagina_igual_h1}")
                ...
                print(f"total_titulos_pagina_duplicado   : {total_titulos_pagina_duplicado }") 
                ...
                print(f"total_meta_description_falta    : {total_meta_description_falta}")  
                ...
                print(f"total_h2_mas_70_caracteres : {total_h2_mas_70_caracteres}")
                ...
                if most_recent_id_escaneo is not None:
                    ...
                    for resultado in resultados_errores_ortograficos:
                        ...
                        if response.status_code == 200:
                            ...
                            palabras_por_frase = [len(frase.split()) for frase in frases if frase.strip()]
                            media_palabras_frase = sum(palabras_por_frase) / len(palabras_por_frase) if palabras_por_frase else 0
                            ...
                            try:
                                total_palabras = len(re.findall(r'\b\w+\b', texto_visible))
                                total_oraciones = resultado.frases
                                #total_silabas = sum([textstat.syllable(word) for word in re.findall(r'\b\w+\b', texto_visible)])
                                total_silabas = sum([textstat.lexicon_count(word, True) for word in re.findall(r'\b\w+\b', texto_visible)])
                                flesh_score = 206.835 - 1.015 * (total_palabras / total_oraciones) - 84.6 * (total_silabas / total_palabras)
                                resultado.flesh_score = round(flesh_score, 2)
                            ...
                            domain_folder = os.path.join(base_folder, resultado.dominio)
